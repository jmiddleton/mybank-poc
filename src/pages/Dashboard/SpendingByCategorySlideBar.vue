    ...
    <div class="categoriesList">
      ...
      <div v-for="category in slidebarData" :key="category.id" class="categoryItem sidebarAlert">
        ...
        <span class="notificationIcon thumb-sm">
          <img :src="categoryConfig[category.title].icon" :style="getCSS(category)" />
        </span>
        <small>{{category.title}}</small>
        <div class="sidebarProgress progress-md">
          <div class="progress">
            <div :style="getBarColor(category)"></div>
          </div>
...
export default {
  ...
  methods: {
    getCSS(category) {
      const rgb = hexToRgb(this.categoryConfig[category.title].color);
    ...
    getBarColor(category) {
      return (
        "background-color:" +
        this.categoryConfig[category.title].color +
        "; width:" +
        (category.total * 100) / this.maxvalue +
        "%"
      );
    },
  ...
  computed: {
    ...mapState("analytics", ["monthlySpendings", "isLoadingMontlySpendings", "categoryConfig"])
  },
  watch: {
