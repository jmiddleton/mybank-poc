<template>
  <section :class="{ widget: true, className }" v-bind="dataWidgster" ref="widget">
    ...
    <div v-if="!customControls && mainControls" class="widgetControls widget-controls">
      ...
      <a v-if="navigate" :id="`leftId-${randomId}`" @click="previousWidget()">
        <i class="la la-angle-left"/>
        <b-tooltip
          v-if="showTooltip"
          :placement="tooltipPlacement"
          :target="`leftId-${randomId}`"
        >Previous</b-tooltip>
      </a>
      <a v-if="navigate" :id="`currentId-${randomId}`" @click="currentWidget()">
        <i class="glyphicon glyphicon-th"/>
        <b-tooltip
          v-if="showTooltip"
          :placement="tooltipPlacement"
          :target="`currentId-${randomId}`"
        >Current</b-tooltip>
      </a>
      <a v-if="navigate" :id="`rightId-${randomId}`" @click="nextWidget()">
        <i class="la la-angle-right"/>
        <b-tooltip
          v-if="showTooltip"
          :placement="tooltipPlacement"
          :target="`rightId-${randomId}`"
        >Next</b-tooltip>
      </a>
...
export default {
  ...
  props: {
    ...
    navigate: { type: [Boolean, String], default: false },
    className: { default: "" },
    title: { default: "" },
